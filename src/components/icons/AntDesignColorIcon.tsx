import { Icon, type IconProps } from "@chakra-ui/react";
import { useId } from "react";

export const AntDesignColorIcon: React.FC<IconProps> = (props) => {
  const id = useId();
  const [antDesignA, antDesignB, antDesignC, antDesignD] = [
    `ant-design-a-${id}`,
    `ant-design-b-${id}`,
    `ant-design-c-${id}`,
    `ant-design-d-${id}`,
  ];
  return (
    <Icon {...props}>
      <svg viewBox="0.6556655367230864 -0.5380000000000003 200.0043344632769 199.88286999097622">
        <defs>
          <linearGradient
            id={antDesignA}
            x1="62.102%"
            x2="108.197%"
            y1="0%"
            y2="37.864%"
          >
            <stop offset="0" stopColor="#4285eb" />
            <stop offset="1" stopColor="#2ec7ff" />
          </linearGradient>
          <linearGradient
            id={antDesignB}
            x1="69.644%"
            x2="54.043%"
            y1="0%"
            y2="108.457%"
          >
            <stop offset="0" stopColor="#29cdff" />
            <stop offset=".379" stopColor="#148eff" />
            <stop offset="1" stopColor="#0a60ff" />
          </linearGradient>
          <linearGradient
            id={antDesignC}
            x1="69.691%"
            x2="16.723%"
            y1="-12.974%"
            y2="117.391%"
          >
            <stop offset="0" stopColor="#fa816e" />
            <stop offset=".415" stopColor="#f74a5c" />
            <stop offset="1" stopColor="#f51d2c" />
          </linearGradient>
          <linearGradient
            id={antDesignD}
            x1="68.128%"
            x2="30.44%"
            y1="-35.691%"
            y2="114.943%"
          >
            <stop offset="0" stopColor="#fa8e7d" />
            <stop offset=".513" stopColor="#f74a5c" />
            <stop offset="1" stopColor="#f51d2c" />
          </linearGradient>
        </defs>
        <g fill="none" fillRule="evenodd">
          <g fillRule="nonzero">
            <path
              d="M91.588 4.177L4.18 91.513a11.981 11.981 0 0 0 0 16.974l87.408 87.336a12.005 12.005 0 0 0 16.989 0l36.648-36.618c4.209-4.205 4.209-11.023 0-15.228-4.208-4.205-11.031-4.205-15.24 0l-27.783 27.76c-1.17 1.169-2.945 1.169-4.114 0l-69.802-69.744c-1.17-1.169-1.17-2.942 0-4.11l69.802-69.745c1.17-1.169 2.944-1.169 4.114 0l27.783 27.76c4.209 4.205 11.032 4.205 15.24 0s4.209-11.022 0-15.227L108.581 4.056c-4.719-4.594-12.312-4.557-16.993.12z"
              fill={`url(#${antDesignA})`}
            />
            <path
              d="M91.588 4.177L4.18 91.513a11.981 11.981 0 0 0 0 16.974l87.408 87.336a12.005 12.005 0 0 0 16.989 0l36.648-36.618c4.209-4.205 4.209-11.023 0-15.228-4.208-4.205-11.031-4.205-15.24 0l-27.783 27.76c-1.17 1.169-2.945 1.169-4.114 0l-69.802-69.744c-1.17-1.169-1.17-2.942 0-4.11l69.802-69.745c2.912-2.51 7.664-7.596 14.642-8.786 5.186-.883 10.855 1.062 17.009 5.837L108.58 4.056c-4.719-4.594-12.312-4.557-16.993.12z"
              fill={`url(#${antDesignB})`}
            />
            <path
              d="M153.686 135.855c4.208 4.205 11.031 4.205 15.24 0l27.034-27.012c4.7-4.696 4.7-12.28 0-16.974l-27.27-27.15c-4.218-4.2-11.043-4.195-15.254.013-4.209 4.205-4.209 11.022 0 15.227l18.418 18.403c1.17 1.169 1.17 2.943 0 4.111l-18.168 18.154c-4.209 4.205-4.209 11.023 0 15.228z"
              fill={`url(#${antDesignC})`}
            />
          </g>
          <ellipse
            cx="100.519"
            cy="100.437"
            fill={`url(#${antDesignD})`}
            rx="23.6"
            ry="23.581"
          />
        </g>
      </svg>
    </Icon>
  );
};
